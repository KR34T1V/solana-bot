...
describe('GET /api/backtest/[id]/status', () => {
  const mockParams = {
    id: 'test-backtest-id'
  };
  ...
  it('should return backtest status and results for authorized user', async () => {
    const mockBacktest = {
      id: 'test-backtest-id',
      userId: 'test-user-id',
      strategyId: 'test-strategy-id',
    ...
    expect(data).toEqual({
      status: mockBacktest.status,
      results: mockBacktest.results
    });
